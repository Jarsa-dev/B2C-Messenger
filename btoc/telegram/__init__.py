        ...
        def process_confirmacion_step(message):
            if message.text == 'SI':
                ...
                BOT.send_message(
                    ...
                    reply_markup=markup)
            if message.text == 'NO':
                BOT.send_message(message.chat.id, '¿Que dato esta incorrecto?')
                reg1 = types.KeyboardButton('Razon social')
                reg2 = types.KeyboardButton('RFC')
                reg3 = types.KeyboardButton('Direccion')
                reg4 = types.KeyboardButton('Regimen fiscal')
                markup.row(reg1)
                markup.row(reg2)
                markup.row(reg3)
                markup.row(reg4)
